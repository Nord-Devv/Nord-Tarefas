...
from .models import Funcionario  # Replace 'your_app' with actual app name
...
class FuncionarioAPITestCase(TestCase):
    ...
    def test_login_funcionario_success(self):
        response = self.client.post(self.login_url, {"email_funcionario": "test@example.com", "senha_funcionario": "password123"}, format='json')
        self.assertEqual(response.status_code, 200)
        self.assertIn("token", response.json())
    
    def test_login_funcionario_invalid_credentials(self):
        response = self.client.post(self.login_url, {"email_funcionario": "test@example.com", "senha_funcionario": "wrongpassword"}, format='json')
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.json(), {"error": "Senha incorreta"})

    def test_login_funcionario_user_not_found(self):
        response = self.client.post(self.login_url, {"email_funcionario": "nonexistent@example.com", "senha_funcionario": "password123"}, format='json')
        self.assertEqual(response.status_code, 400)
