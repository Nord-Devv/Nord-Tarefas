...
from ninja.errors import (AuthenticationError, ConfigError, Throttled,
                          ValidationError)
...
class Operation:
    def __init__(
        ...
        *,
        ...
        self.auth_param: Sequence[Callable] | Callable | object | None = auth
    ...
    def _set_auth(self, auth: Sequence[Callable] | Callable | object | None) -> None:
...
class AsyncOperation(Operation):
    ...
    async def _run_authentication(self, request: HttpRequest) -> HttpResponse | None:  # type: ignore
        for callback in self.auth_callbacks:
            try:
                if is_async_callable(callback) or getattr(callback, "is_async", False):
                    cor: Coroutine | None = callback(request)
                    if cor is None:
                        result = None
                    else:
                        result = await cor
